...
export const DataProvider = ({ children }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const isMockSession = Boolean(session?.isMock) || Boolean(session?.user?.id?.startsWith('demo-user-'));

      const finalInsts = (!isMockSession && insts) ? insts : INITIAL_INSTRUMENTS;
      const finalApps = (!isMockSession && appsWithCerts) ? appsWithCerts : INITIAL_APPLICATIONS;
      const finalCerts = (!isMockSession && certs) ? certs : INITIAL_CERTIFICATES;
      const finalOffs = (!isMockSession && offs) ? offs : MOCK_OFFICERS;
